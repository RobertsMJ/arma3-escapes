...
_extractionGuard = {
	...
	waitUntil{
		sleep 0.5;
		!(alive _boat1) and !(alive _boat2) and !(alive _boat3)
	};
...
while {
	{
		(_x in _boat1) || (_x in _boat2) || (_x in _boat3)
	} count (call A3E_fnc_GetPlayers) != count(call A3E_fnc_GetPlayers)
} do {
	sleep 1;
};
...
if({
	vehicle _x == _boat1 || vehicle _x == _boat2 || vehicle _x == _boat3
} count (call A3E_fnc_GetPlayers) == count (call A3E_fnc_GetPlayers)) then {
