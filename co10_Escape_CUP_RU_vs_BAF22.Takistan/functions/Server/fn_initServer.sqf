...
[A3E_StartPos, _backPack, _enemyFrequency] spawn {
    ...
    for [{_i = 0}, {_i < _guardCount}, {_i = _i + 1}] do {
        ...
        _group = _guardGroup createUnit [(a3e_arr_Escape_StartPositionGuardTypes select floor (random count a3e_arr_Escape_StartPositionGuardTypes)), _pos, [], 0, "FORM"];
		[_group] joinSilent _guardGroup;

        if (count units _guardGroup >= 3) then {
            _createNewGroup = true;
        };
    };
